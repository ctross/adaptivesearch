...
for(q in 33){                                                               # Run the simulation several times
...
df3 <- data.frame(X=loc.x[which(Hits==1)],Y=loc.y[which(Hits==1)],Prey=rep("Encountered",length(loc.y[which(Hits==1)])) )
 ggplot(data=df) +
     ...
     xlim(-4500,4500) + ylim(-4500,4500) + theme(legend.text=element_text(size=14)) + theme(text = element_text(size=16))
     ggsave("T-D-H.pdf",height=6, width=8)