...
X_Mushrooms[[i]]<-rnorm(N_PerPatch[i], runif(1,-2500,2500), rpois(1, 350))+100000
Y_Mushrooms[[i]]<-rnorm(N_PerPatch[i], runif(1,-2500,2500), rpois(1, 350))+100000
}
...
AlphaDist <- Phi0L        # Use Levy intecepts,
AlphaAngle <- Psi0L        

BetaDist  <- PhiA         # but adaptive Betas
BetaAngle <- PsiA

SDDist <- OmegaL          # and Levy other
DAngle <- EtaL                                                               
...
for(i in 1:N_Patches){
X_Mushrooms[[i]]<-rnorm(N_PerPatch[i], runif(1,-2500,2500), rpois(1, 350))+100000  # Make some patches of prey, but really far away
Y_Mushrooms[[i]]<-rnorm(N_PerPatch[i], runif(1,-2500,2500), rpois(1, 350))+100000  #
}
...
################################################################################ Store Each Simulation Here
 StoreHits.D.H.E2 <- StoreHits   
 StoreAngDiff.D.H.E2 <- StoreAngDiff
 StoreSpeed.D.H.E2  <- StoreSpeed
 
 ############################################################# Full Distributions  Example 2
################################################################################
DF.DH<-melt(as.data.frame((StoreSpeed.D.H.E2)))
...
DF.Dh$value <- DF.Dh$value*runif(length(DF.Dh$value),0.95,1/0.95)  # Jitter
...
ggplot(DF1) + geom_line(aes(x = value, group = ID,color=Search_Mode),stat='density',alpha=I(0.25),bw=10) + labs(x = "Step-size",y="Density") +
      ...
      ggsave("P-D-S-2.pdf",height=6, width=8)                            
                     
################################################################################
DF.DH<-melt(as.data.frame((StoreAngDiff.D.H.E2)))
...
g 
  ...
  ggsave("P-D-A-2.pdf",g,height=6, width=8) 
