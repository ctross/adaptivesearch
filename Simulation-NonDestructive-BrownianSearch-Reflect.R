...
for(k in 1:Lags){                                                               #    
 ...
 R<- rtnorm(1,PredDist,SDDist,0,)*7.5                                         # Then simulate a step distance. 
...
for(k in 1:Lags){                                                               #    
 ...
 loc.x[s]<-ifelse(loc.x[s-1]+xnew < 4000  & loc.x[s-1]+xnew > -4000, loc.x[s-1]+xnew, 
                   ifelse(loc.x[s-1]+xnew > 4000, -4000 + ((loc.x[s-1]+xnew) - 4000),  
                          4000 - (abs(loc.x[s-1]+xnew) - 4000)))
                          
 loc.y[s]<-ifelse(loc.y[s-1]+ynew < 4000  & loc.y[s-1]+ynew > -4000, loc.y[s-1]+ynew, 
                   ifelse(loc.y[s-1]+ynew > 4000, -4000 + ((loc.y[s-1]+ynew) - 4000),  
                          4000 - (abs(loc.y[s-1]+ynew) - 4000)))
...
################################################################################ Store Each Simulation Here
 Res.d.B<-cbind(MeanSpeed, MeanDHeading, MeanHits, SDSpeed,DDHeading,SDHits) 

 StoreHits.d.B <- StoreHits   
 StoreAngDiff.d.B <- StoreAngDiff
 StoreSpeed.d.B  <- StoreSpeed
